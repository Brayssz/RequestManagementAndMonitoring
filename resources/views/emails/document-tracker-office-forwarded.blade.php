@php
    // Receiving-office notification: sent to every user attached to the office a
    // document was forwarded or returned to. Mirrors the layout and palette of the
    // requestor movement email (document-tracker-transmitted) so all tracker mails
    // look identical; only the wording and the details rows differ.
    ...
    $headerTint = $isReturned ? '#f6dcdc' : '#dbe6fb';
    $headline = $isReturned ? 'Document Returned to Your Office' : 'Document Forwarded to Your Office';
    $actionText = $isReturned ? 'returned' : 'forwarded';
    $statusLabel = $isReturned ? 'Returned' : 'Forwarded';
    $actorLabel = $isReturned ? 'Returned By' : 'Forwarded By';
    $destinationLabel = $isReturned ? 'Returned To' : 'Forwarded To';

    $toOfficeName = $toOffice->name ?? optional($documentTracker->currentOffice)->name ?? 'your office';
    $fromOfficeName = optional($fromOffice)->name ?? '-';
    $forwardedByName = optional($forwardedBy)->name ?? '-';
                    ...
                    <tr>
                        <td style="padding:32px 32px 8px;">
                            <p style="margin:0 0 16px; font-size:15px; color:#2d3748;">
                                Dear <strong>{{ $recipient->name ?? 'Colleague' }}</strong>,
                            </p>

                            <p style="margin:0 0 24px; font-size:15px; line-height:1.6; color:#4a5568;">
                                A document with tracking number
                                <strong>{{ $documentTracker->tracking_number }}</strong>
                                has been {{ $actionText }} to <strong>{{ $toOfficeName }}</strong>.
                                Please receive and process it accordingly. Its details are shown below.
                            </p>

                            <!-- Tracking number highlight -->
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
                                <tr>
                                    <td style="background-color:{{ $accentBg }}; border:1px solid {{ $accentBorder }}; border-radius:8px; padding:16px 20px;" align="center">
                                        <p style="margin:0; font-size:11px; letter-spacing:1px; text-transform:uppercase; color:#5a6b85;">Tracking Number</p>
                                        <p style="margin:4px 0 0; font-size:24px; font-weight:bold; letter-spacing:1px; color:{{ $accentColor }};">
                            ...
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e2e8f0; border-radius:8px; border-collapse:separate; overflow:hidden;">
                                ...
                                <tr>
                                    <td style="padding:12px 16px; font-size:13px; color:#5a6b85; background-color:#f8fafc; border-bottom:1px solid #edf2f7;"><strong>Requestor</strong></td>
                                    <td style="padding:12px 16px; font-size:13px; color:#2d3748; border-bottom:1px solid #edf2f7;">{{ $documentTracker->requestor_name ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <td style="padding:12px 16px; font-size:13px; color:#5a6b85; background-color:#f8fafc; border-bottom:1px solid #edf2f7;"><strong>From Office</strong></td>
                                    <td style="padding:12px 16px; font-size:13px; color:#2d3748; border-bottom:1px solid #edf2f7;">{{ $fromOfficeName }}</td>
                                </tr>
                                <tr>
                                    <td style="padding:12px 16px; font-size:13px; color:#5a6b85; background-color:#f8fafc; border-bottom:1px solid #edf2f7;"><strong>{{ $destinationLabel }}</strong></td>
                                    <td style="padding:12px 16px; font-size:13px; color:#2d3748; border-bottom:1px solid #edf2f7;">{{ $toOfficeName }}</td>
                                </tr>
                                <tr>
                                    <td style="padding:12px 16px; font-size:13px; color:#5a6b85; background-color:#f8fafc; border-bottom:1px solid #edf2f7;"><strong>{{ $actorLabel }}</strong></td>
                                    <td style="padding:12px 16px; font-size:13px; color:#2d3748; border-bottom:1px solid #edf2f7;">{{ $forwardedByName }}</td>
                                </tr>
                                @if ($trimmedNotes !== '')
                                <tr>
                                    <td style="padding:12px 16px; font-size:13px; color:#5a6b85; background-color:#f8fafc; border-bottom:1px solid #edf2f7;" valign="top"><strong>Notes</strong></td>
                                    <td style="padding:12px 16px; font-size:13px; color:#2d3748; line-height:1.6; border-bottom:1px solid #edf2f7;">{!! nl2br(e($trimmedNotes)) !!}</td>
                                </tr>
                                @endif
                                <tr>
                                    <td style="padding:12px 16px; font-size:13px; color:#5a6b85; background-color:#f8fafc;"><strong>Status</strong></td>
                                    <td style="padding:12px 16px; font-size:13px;">
                                        <span style="display:inline-block; padding:3px 12px; font-size:12px; font-weight:bold; color:{{ $accentColor }}; background-color:{{ $accentSoft }}; border-radius:999px;">{{ $statusLabel }}</span>
                                    </td>
                                </tr>
                            </table>
